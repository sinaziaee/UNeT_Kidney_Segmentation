...
class EffUNet(nn.Module):
    ...
    def __init__(self, start_ch, num_ch, use_xavier=True, use_batchNorm=True, dropout=0.4, retain_size=True,
                nbCls=2, retainDim=True, outsize=(IMG_HEIGHT, IMG_HEIGHT)):
        super().__init__()

        self.start_conv = nn.Sequential(
            nn.Conv2d(in_channels=start_ch, out_channels=32, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU()
        )
        ...
        self.up_block_4 = DecoderBlock(432, 256, dropout=dropout)

        self.up_block_3 = DecoderBlock(296, 128, dropout=dropout)

        self.up_block_2 = DecoderBlock(152, 64, dropout=dropout)

        self.up_block_1a = DecoderBlock(96, 32, dropout=dropout)

        self.up_block_1b = DecoderBlock(32, 16, dropout=dropout)

        self.head_conv = nn.Conv2d(in_channels=16, out_channels=nbCls, kernel_size=3, padding=1, bias=False)
